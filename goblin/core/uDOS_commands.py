...
class CommandHandler:
    ...
    def __init__(
        ...
        from dev.goblin.core.commands.panel_handler import PanelCommandHandler
        ...
        from dev.goblin.core.commands.maintenance_handler import MaintenanceHandler
        ...
        self.map_handler = (
            MapCommandHandler(**handler_kwargs) if MapCommandHandler else None
        )
        ...
        self.maintenance_handler = MaintenanceHandler(**handler_kwargs)
        ...
        try:
            ...
            # DRAW - 24×24 Tile Editor (v1.2.31)
                ...
                if self.map_handler:
                    ...
                    return (
                        "❌ MAP commands require Play extension\n"
            ...
            # Maintenance Operations (TIDY, CLEAN) - Alpha v1.0.0.0+
                ...
                return self.maintenance_handler.handle_tidy(
                    ([command] + params) if command else params, grid, parser
                )

            elif module == "CLEAN":
                return self.maintenance_handler.handle_clean(
                    ([command] + params) if command else params, grid, parser
                )
            ...
            # v1.2.2 - DEV MODE Interactive Debugging
                ...
                # Route to appropriate handler based on next parameter
                if command and command.upper() == "MODE":
                    # DEV MODE ON/OFF/STATUS/HELP → system_handler → environment_handler
                    return self.system_handler.handle(
                        "DEV", [command] + params, grid, parser
                    )
                else:
                    # DEV ENABLE/BREAK/STEP/etc → dev_mode_handler (script debugging)
