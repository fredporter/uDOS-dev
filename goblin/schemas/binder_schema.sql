...
CREATE TABLE IF NOT EXISTS binders (
    ...
    name TEXT NOT NULL,
    description TEXT,
    status TEXT NOT NULL DEFAULT 'draft',  -- draft, in-progress, review, complete
...
CREATE TABLE IF NOT EXISTS chapters (
    ...
    chapter_id TEXT NOT NULL,  -- User-facing identifier
    title TEXT NOT NULL,
    content TEXT NOT NULL DEFAULT '',
    
    -- Ordering and status
    order_index INTEGER NOT NULL DEFAULT 0,
    status TEXT NOT NULL DEFAULT 'draft',  -- draft, review, complete
...
CREATE TABLE IF NOT EXISTS outputs (
    id TEXT PRIMARY KEY,
    binder_id TEXT NOT NULL,
    format TEXT NOT NULL,  -- markdown, pdf, json, brief
    ...
    -- Status
    status TEXT NOT NULL DEFAULT 'generated',  -- generated, verified, published
...
-- ========================================
-- Binder Metadata Table (Key-Value Store)
-- ========================================
...
-- Update binder updated_at on changes
CREATE TRIGGER IF NOT EXISTS update_binder_timestamp 
AFTER UPDATE ON binders
BEGIN
    UPDATE binders SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
END;

-- Update chapter updated_at on changes
...
-- Update binder total_chapters count
CREATE TRIGGER IF NOT EXISTS update_binder_chapter_count_insert
AFTER INSERT ON chapters
BEGIN
    UPDATE binders 
    SET total_chapters = (SELECT COUNT(*) FROM chapters WHERE binder_id = NEW.binder_id)
    WHERE id = NEW.binder_id;
END;

CREATE TRIGGER IF NOT EXISTS update_binder_chapter_count_delete
AFTER DELETE ON chapters
BEGIN
    UPDATE binders 
    SET total_chapters = (SELECT COUNT(*) FROM chapters WHERE binder_id = OLD.binder_id)
    WHERE id = OLD.binder_id;
END;

-- Update binder word count
CREATE TRIGGER IF NOT EXISTS update_binder_word_count
AFTER UPDATE ON chapters
WHEN NEW.word_count != OLD.word_count
BEGIN
    UPDATE binders 
    SET total_words = (SELECT SUM(word_count) FROM chapters WHERE binder_id = NEW.binder_id)
    WHERE id = NEW.binder_id;
END;
